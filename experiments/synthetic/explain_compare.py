...
def gradexp_explain(gradexp, image_t):
    ...
    image_t = image_t.unsqueeze(0)
    t0 = time.time()
    explanation, _ = gradexp.shap_values(image_t, ranked_outputs=1, nsamples=200)
    t = time.time()
    runtime = round(t - t0, 6)
...
for n in c:
    for exp in exp_mapper:
        ...
        for i, image_path in enumerate(
            true_positives[n] if not RANDOMIZE_MODEL else true_positives[n][:4]
        ):
            ...
            print(
                f"{exp_name}: {i+1}/{len(true_positives[n])}({n}) runtime={runtime:4f} s"
            )
            np.save(os.path.join(explainer_dir, image_name), explanation)

        np.save(os.path.join(explainer_dir, f"comp_times_{n}.npy"), comp_times)
